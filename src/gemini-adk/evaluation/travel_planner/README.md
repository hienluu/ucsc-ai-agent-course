...
cd Hands-On-AI-Engineering/ai_agents/adk_travel_planner_agent
cp .env.example .env
```

Add your API keys to `.env` (see [Environment Variables](#environment-variables) below), then install dependencies:

```bash
uv sync
```

`uv sync` creates a `.venv/` in the project, installs everything from `pyproject.toml`, and writes a `uv.lock` for reproducible installs.

## Environment Variables

| Variable | Description | Where to Get It |
|----------|-------------|-----------------|
| `GOOGLE_API_KEY` | Authenticates Gemini calls via the Google GenAI SDK that ADK uses under the hood | [Google AI Studio](https://aistudio.google.com/app/apikey) → API keys |
...
| `ADK_MODEL` *(optional)* | Override the Gemini model used by the agent (default `gemini-2.5-flash`) | n/a |
